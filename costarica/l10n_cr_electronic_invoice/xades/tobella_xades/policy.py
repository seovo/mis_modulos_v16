...
from .constants import MAP_HASHLIB, NS_MAP
from .ns import EtsiNS
from .utils import dict_compare, rdns_to_map
...
class Policy(object):
    ...
    def calculate_certificate(self, node, key_x509):
        ...
        create_node("X509IssuerName", issuer_serial, DSigNs).text = get_rdns_name(
            key_x509.issuer.rdns
        )
        create_node("X509SerialNumber", issuer_serial, DSigNs).text = str(key_x509.serial_number)
        return
    ...
    def validate_certificate(self, node, signature):
        ...
        if serial is not None:
            ...
            for cert in certs:
                if (
                    cert.find("etsi:IssuerSerial/ds:X509IssuerName", namespaces=NS_MAP).text
                    == serial_name
                    and cert.find("etsi:IssuerSerial/ds:X509SerialNumber", namespaces=NS_MAP).text
                    == serial_number
                ):
                    certificate = cert
        ...
        if x509_data is not None:
            parsed_x509 = load_der_x509_certificate(b64decode(x509_data.text), default_backend())
            assert (
                str(parsed_x509.serial_number)
                == certificate.find("etsi:IssuerSerial/ds:X509SerialNumber", namespaces=NS_MAP).text
            )
            dict_compare(
                rdns_to_map(get_rdns_name(parsed_x509.issuer.rdns)),
                rdns_to_map(
                    certificate.find("etsi:IssuerSerial/ds:X509IssuerName", namespaces=NS_MAP).text
                ),
            )
            ...
            assert (
                b64encode(
                    parsed_x509.fingerprint(
                        MAP_HASHLIB[
                            digest.find("ds:DigestMethod", namespaces=NS_MAP).get("Algorithm")
                        ]()
                    )
                )
                == digest.find("ds:DigestValue", namespaces=NS_MAP).text.encode()
            )
...
class PolicyId(Policy):
    ...
    def calculate_policy_node(self, node, sign=False):
        ...
        value = urllib.request.urlopen(remote).read()
        ...
        if sign:
            ...
            digest_value = policy_id.find("etsi:SigPolicyHash/ds:DigestValue", namespaces=NS_MAP)
