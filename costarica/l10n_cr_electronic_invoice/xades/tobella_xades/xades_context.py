...
class XAdESContext(SignatureContext):
    ...
    def verify(self, node):
        ...
        :return:
        """
        schema = etree.XMLSchema(etree.parse(path.join(path.dirname(__file__), "data/XAdES.xsd")))
    ...
    def calculate_signature_properties(self, signature_properties, node, sign=False):
        ...
        certificate_list = signature_properties.find("etsi:SigningCertificate", namespaces=NS_MAP)
        ...
        policy = signature_properties.find("etsi:SignaturePolicyIdentifier", namespaces=NS_MAP)
