...
class ReportPle08(models.Model):
    _name = 'report.ple.06.1'
    _inherit = ['report.ple']
    _description = 'Registro mayor'
    ...
    line_ids = fields.One2many(comodel_name='report.ple.06.1.line', inverse_name='ple_id', string='Detalle del libro',
                               readonly=True)
    ...
    def action_generate(self):
        ...
        if self.type_report in ['normal']:
            ...
            filename = template.format(
                prefix, company_vat, year, month, '060100', self.indicator_operation,
    ...
    def create_lines(self, invoice_obj):
        ...
        for x, line in enumerate(invoice_obj, 1):
            self.env['report.ple.06.1.line'].create({
    ...
    def _get_content(move_line_obj, v_anio, v_mes):
        ...
        template = '{period}|{cuo}|{move_name}|{codigo_cuenta}|{codigo_unidad_op}|{codigo_centro_cos}|' \
                   '{tipo_moneda}|{tipo_doc_iden_emisor}|{num_doc_iden_emisor}|' \
                   '{tipo_comprobante_pago}|{num_serie_comprobante_pago}|{num_comprobante_pago}|' \
...
class ReportPle08Line(models.Model):
    _name = 'report.ple.06.1.line'
    _order = 'fecha_contable'
    _description = 'Detalle de libro mayor'

    move_line_id = fields.Many2one(comodel_name='account.move.line', string='Apunte', required=True)
    ...
    codigo_cuenta_desagregado_id = fields.Many2one("account.account", string="Cuenta contable desagregado",
    ...
    tipo_doc_iden_emisor = fields.Char(string="Tipo Documento Identidad Emisor", compute='_compute_data', readonly=True)
    num_doc_iden_emisor = fields.Char(string="Número Documento Identidad Emisor", compute='_compute_data', readonly=True)
    ...
    num_serie_comprobante_pago = fields.Char(string="Número serie Comprobante Pago",
                                             compute='_compute_data',
                                             )  # , inverse= '_inverse_compute_campo_num_serie_comprobante_pago', store = True , readonly=False )
    num_comprobante_pago = fields.Char(string="Número Comprobante de Pago",
                                       compute='_compute_data',
                                       )  # inverse= '_inverse_compute_campo_num_comprobante_pago' ,store = True ,  readonly=False)
    ...
    ple_id = fields.Many2one(comodel_name='report.ple.06.1')

    @api.depends('move_line_id')
    def _compute_data(self):
        def get_series_correlative(move_line_id):
            if move_line_id.move_id.move_type in ['in_invoice', 'in_refund']:
