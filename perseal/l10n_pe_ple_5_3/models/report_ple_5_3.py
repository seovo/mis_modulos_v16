...
class ReportPle053(models.Model):
    _name = 'report.ple.05.3'
    _inherit = ['report.ple']
    _description = 'Registro Diario'

    line_ids = fields.One2many(comodel_name='report.ple.05.3.line', inverse_name='ple_id', string='Detalle del libro', readonly=True)
    ...
    def create(self, vals):
        res = super(ReportPle053, self).create(vals)
    ...
    def create_lines(self, moves_obj):
        ...
        for x, line in enumerate(moves_obj, 1):
            self.env['report.ple.05.3.line'].create({
                'account_id': line.id,
                'ple_id': self.id,
...
class ReportPle053Line(models.Model):
    _name = 'report.ple.05.3.line'
    ...
    ple_id = fields.Many2one(comodel_name='report.ple.05.3')
    ...
    def _compute_data(self):
        ...
        config_settings = self.env['ple.configuration'].search([('report_type', '=', '5.3'), ('company_id', '=', self.ple_id.company_id.id)])
        self.mapped(lambda x: x.update({
            ...
            'codigo_cuenta_tributario': config_settings.account_chart_code,
