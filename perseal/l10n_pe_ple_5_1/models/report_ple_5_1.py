...
class ReportPle08(models.Model):
    ...
    line_ids = fields.One2many(comodel_name='report.ple.05.1.line', inverse_name='ple_id', string='Detalle del libro',
                               readonly=True)
    ...
    def create_lines(self, invoice_obj):
        ...
        for x, line in enumerate(invoice_obj, 1):
            self.env['report.ple.05.1.line'].create({
...
class ReportPle08Line(models.Model):
    ...
    tipo_doc_iden_emisor = fields.Char(string="Tipo Documento Identidad Emisor",
                                       compute='_compute_data', readonly=True)
    ...
    ple_id = fields.Many2one(comodel_name='report.ple.05.1')
