...
class CulquiCommon(PaymentAcquirerCommon):

    def setUp(self):
        super().setUp()
        self.culqui = self.env.ref('payment_culqi.payment_acquirer_culqui')


@odoo.tests.common.at_install(False)
@odoo.tests.common.post_install(True)
class CulquiTest(CulquiCommon):

    @unittest.skip("Culqui test disabled: We do not want to overload Culqui with runbot's requests")
    def test_10_culqui_s2s(self):
        self.assertEqual(self.culqui.environment, 'test', 'test without test environment')

        # Add Culqui credentials
        # establish from demo data

        # Create payment meethod for Culqui
        payment_token = self.env['payment.token'].create({
            'acquirer_id': self.culqui.id,
    ...
    def test_20_culqui_form_render(self):
        self.assertEqual(self.culqui.environment, 'test', 'test without test environment')
        ...
        # render the button
        res = self.culqui.render('SO404', 320.0, self.currency_euro.id, values=self.buyer_values).decode('utf-8')
    ...
    @unittest.skip("Stripe test disabled: We do not want to overload Stripe with runbot's requests")
    def test_30_culqui_form_management(self):
        self.assertEqual(self.culqui.environment, 'test', 'test without test environment')
