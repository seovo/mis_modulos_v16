...
class FollowupManualReminderOperation(models.TransientModel):
    _name = 'account_followup.manual_reminder.operation'
    ...
    def default_get(self, fields_list):
        ...
        partner = self.env[self.env.context['active_model']].browse(self.env.context['active_id']).beneficiario_id
        # partner = self.env['res.partner'].browse(self.env.context['beneficiario_id'])
        partner.ensure_one()
        ...
        defaults.update(
            ...
            template_id = self.env.ref('account_followup.email_template_followup_1').id,
        )
        return defaults
    ...
    email_recipient_ids = fields.Many2many(string="Extra Recipients", comodel_name='res.partner',
                                           relation='rel_followup_manual_reminder_operation_res_partner')  # override
