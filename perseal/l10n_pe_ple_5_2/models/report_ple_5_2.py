...
class ReportPle08(models.Model):
    _name = 'report.ple.05.2'
    _inherit = ['report.ple']
    _description = 'Registro Diario simplificado'
    ...
    line_ids = fields.One2many(comodel_name='report.ple.05.2.line', inverse_name='ple_id', string='Detalle del libro',
                               readonly=True)
    ...
    def create_lines(self, invoice_obj):
        ...
        for x, line in enumerate(invoice_obj, 1):
            self.env['report.ple.05.2.line'].create({
...
class ReportPle08Line(models.Model):
    ...
    num_comprobante_pago = fields.Char(string="Número Comprobante de Pago", compute='_compute_data')
    ...
    ple_id = fields.Many2one(comodel_name='report.ple.05.2')
    ...
    def _compute_data(self):
        def get_series_correlative(move_line_id):
            if move_line_id.move_id.move_type in ['in_invoice', 'in_refund']:
                ...
                return (name.split('-')[0], name.split('-')[1]) if name and '-' in name else ('', '')
            elif move_line_id.move_id.move_type in ['entry']:
                name = move_line_id.move_id.ref
                return ('', name)
