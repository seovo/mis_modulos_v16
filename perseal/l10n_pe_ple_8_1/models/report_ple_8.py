...
class ReportPle08Line(models.Model):
    ...
    def _compute_data(self):
        ...
        self.mapped(lambda x: x.update({
            ...
            'document_payment_type_update': (x.invoice_id.move_type == 'in_refund' and x.invoice_id.reversed_entry_id.l10n_latam_document_type_id.code or
    ...
    def _compute_amount(self):
        ...
        def get_amount_tax(invoice):
            ...
            config_settings = self.env['ple.configuration'].search([('report_type', '=', '8.1'), ('company_id', '=', self.ple_id.company_id.id)])
            data_process = self._l10n_pe_edi_get_edi_values(invoice)
            v_base_imponible = 0
            v_monto_exo = 0
            totalVentaGravada = 0
            totalVentaGravada2 = 0
            totalVentaGravada3 = 0
            ...
            for element in data_process['tax_details']['grouped_taxes']:
                if element.get('tax_id') in config_settings.l10n_pe_txbl_txbl_vat.ids:
                    totalVentaGravada += element.get('base')
                    sumatoriaIgv += element.get('amount')
                if element.get('tax_id') in config_settings.l10n_pe_txbl_txbl_notxbl_vat.ids:
                    totalVentaGravada2 += element.get('base')
                    sumatoriaIgv2 += element.get('amount')
                if element.get('tax_id') in config_settings.l10n_pe_txbl_notxbl_vat.ids:
                    totalVentaGravada3 += element.get('base')
                    sumatoriaIgv3 += element.get('amount')
                if element.get('tax_id') in config_settings.l10n_pe_purchase_other_tax.ids:
                    other += element.get('amount')
                if element.get('tax_id') in config_settings.l10n_pe_notxbl_vat.ids:
            ...
            if invoice.currency_id != invoice.company_id.currency_id:
                ...
                amount_total = invoice.currency_id._convert(amount_total, invoice.company_id.currency_id,
                                                            invoice.company_id, date)
                # exo = exo + v_monto_exo
            return v_base_imponible, igv, totalVentaGravada, totalVentaExonerada, totalVentaInafecta, isc, other, amount_total, totalVentaGravada2, sumatoriaIgv2, totalVentaGravada3, sumatoriaIgv2
        ...
        self.mapped(lambda w: w.update({
            'amount_untaxed1': get_amount_tax(w.invoice_id)[2],
            'amount_tax_igv1': get_amount_tax(w.invoice_id)[1],
            'amount_untaxed2': get_amount_tax(w.invoice_id)[8],
            'amount_tax_igv2': get_amount_tax(w.invoice_id)[9],
            'amount_untaxed3': get_amount_tax(w.invoice_id)[10],
            'amount_tax_igv3': get_amount_tax(w.invoice_id)[11],
            'amount_exo': (get_amount_tax(w.invoice_id)[3]),
