...
class BusinessDocumentImport(models.AbstractModel):
    ...
    def _match_partner(self, partner_dict, chatter_msg, partner_type="supplier"):
        ...
        if partner_dict.get("country_code"):
            ...
            if countries:
                ...
                chatter_msg.append(
                    _(
                        "The analysis of the business document returned '{}' as "
                        "country code. But there are no country with that code "
                        "in Odoo."
                    ).format(partner_dict["country_code"])
                )
        ...
        if partner_dict.get("vat"):
            ...
            partners = rpo.search(domain + [("parent_id", "=", False), ("vat", "=", vat)])
            if partners:
                return partners[0]
            else:
                chatter_msg.append(
                    _(
                        "The analysis of the business document returned '{}' as "
                        "{} VAT number. But there are no {} "
                        "with this VAT number in Odoo."
                    ).format(vat, partner_type_label, partner_type_label)
                )

        partner = self._hook_match_partner(partner_dict, chatter_msg, domain, partner_type_label)
        if partner:
            return partner
        website_domain = False
        email_domain = False
        if partner_dict.get("email") and "@" in partner_dict["email"]:
            partners = rpo.search(domain + [("email", "=ilike", partner_dict["email"])])
            if partners:
                return partners[0]
            else:
        ...
        if website_domain or email_domain:
            ...
            partners = rpo.search(domain + [("website", "=ilike", "%" + partner_domain + "%")])
            if not partners and website_domain:
                partners = rpo.search(domain + [("email", "=ilike", "%@" + website_domain)])
            if partners:
                chatter_msg.append(
                    _(
                        "The {} has been identified by the domain name '{}' "
                        "so please check carefully that the {} is correct."
                    ).format(partner_type_label, partner_domain, partner_type_label)
                )

                return partners[0]
        if partner_dict.get("ref"):
            partners = rpo.search(domain + [("ref", "=", partner_dict["ref"])])
            if partners:
                return partners[0]
        if partner_dict.get("name"):
            partners = rpo.search(domain + [("name", "=ilike", partner_dict["name"])])
            if partners:
                return partners[0]
        raise self.user_error_wrap(
            _(
                "Odoo couldn't find any {} corresponding to the following "
    ...
    def _match_shipping_partner(self, shipping_dict, partner, chatter_msg):
        ...
        if shipping_dict.get("partner"):
            partner = self._match_partner(shipping_dict["partner"], chatter_msg, partner_type=False)
        company_id = self._context.get("force_company") or self.env.user.company_id.id
        domain = [
            "|",
            ("company_id", "=", False),
            ...
            if countries:
                ...
                chatter_msg.append(
                    _(
                        ...
                        "country code. But there are no country with that code "
                        "in Odoo."
                    ).format(address_dict["country_code"])
                )
    ...
    @api.model
    def _match_partner_bank(self, partner, iban, bic, chatter_msg, create_if_not_found=False):
        ...
        try:
            ...
            chatter_msg.append(
                _("IBAN <b{}</b> is not valid, so it has been ignored.").format(iban)
            )
            return False
        company_id = self._context.get("force_company") or self.env.user.company_id.id
        ...
        if bankaccounts:
            ...
            chatter_msg.append(
                _(
                    "The bank account <b>IBAN {}</b> has been automatically added on the supplier <b>{}</b>"
                ).format(iban, partner.name)
            )
            return partner_bank
    ...
    def _match_product(self, product_dict, chatter_msg, seller=False):
        ...
        if code:
            ...
            if seller:
                ...
                if (
                    sinfo
                    and sinfo[0].product_tmpl_id.product_variant_ids
                    and len(sinfo[0].product_tmpl_id.product_variant_ids) == 1
                ):
                    return sinfo[0].product_tmpl_id.product_variant_ids[0]

        uom_id = self.env["uom.uom"].search([("code", "=", product_dict.get("uom"))], limit=1)
        ...
        if product_dict.get("taxes"):
            sales_taxes = [
                x.id
                for x in self._match_taxes(
                    product_dict["taxes"], "", type_tax_use="sale", price_include=None
                )
            ]
            supplier_taxes = [
                x.id
        ...
        logger.error("MAB - New Product: {} ".format(new_vals))
    ...
    def _match_currency(self, currency_dict, chatter_msg):
        ...
        if currency_dict.get("iso"):
            ...
            currencies = rco.search([("name", "=", currency_iso)])
            if currencies:
                return currencies[0]
            else:
                raise self.user_error_wrap(
        ...
        if currency_dict.get("iso_or_symbol"):
            ...
            if currencies:
                ...
                raise self.user_error_wrap(
                    ...
                    ).format(currency_dict["iso_or_symbol"])
                )
                ...
                if country.currency_id:
                    ...
                    raise self.user_error_wrap(
                        _(
                            ...
                            "But the country '{}' doesn't have any related "
        ...
        chatter_msg.append(
            _("No currency specified, so Odoo used the company currency ({})").format(
                company_cur.name
            )
        )
        return company_cur
    ...
    def _match_uom(self, uom_dict, chatter_msg, product=False):
        ...
        if uom_dict.get("unece_code"):
            ...
            uoms = puo.search([("code", "=", uom_dict["unece_code"])])
            if uoms:
                return uoms[0]
            else:
                chatter_msg.append(
                    _(
                        ...
                        "Odoo."
                    ).format(uom_dict["unece_code"])
                )
        if uom_dict.get("name"):
            uoms = puo.search([("name", "=ilike", uom_dict["name"] + "%")])
            if uoms:
                return uoms[0]
        if product:
        ...
        chatter_msg.append(
            _(
                ...
                "<ul><li>UNECE code: {}</li>"
                "<li>Name of the unit of measure: {}</li></ul>"
                "<p>So the unit of measure 'Unit(s)' has been used. <em>You may "
                "have to change it manually.</em></p>"
            ).format(uom_dict.get("unece_code"), uom_dict.get("name"))
        )
        return self.env.ref("product.product_uom_unit")

    @api.model
    def _match_taxes(self, taxes_list, chatter_msg, type_tax_use="purchase", price_include=False):
    ...
    def _match_tax(self, tax_dict, chatter_msg, type_tax_use="purchase", price_include=False):
        ...
        if tax_dict.get("tax_code"):
            domain.append(("tax_code", "=", tax_dict["tax_code"]))
        ...
        raise self.user_error_wrap(
            _(
                "Odoo couldn't find any tax with 'Tax Application' = '{}' "
                "and 'Tax Included in Price' = '{}' which correspond to the "
                "following information extracted from the business document:\n"
                "UNECE Tax Type code: {}\n"
                "UNECE Tax Category code: {}\n"
                "Tax amount: {} {}"
            ).format(
                type_tax_use,
                price_include,
    ...
    def compare_lines(
        ...
        for eline in existing_lines:
            if not eline.get("product"):
                chatter_msg.append(
                    _(
                        "The existing line '{}' doesn't have any product, so <b>the lines haven't been updated</b>."
                    ).format(eline.get("name"))
                )
                return False
            if eline["product"] in existing_lines_dict:
                chatter_msg.append(
                    _(
                        "The product '{}' is used on several existing lines, so <b>the lines haven't been updated</b>."
                    ).format(eline["product"].display_name)
                )
                return False
        ...
        for iline in import_lines:
            if not iline.get("product"):
                chatter_msg.append(
                    _(
                        "One of the imported lines doesn't have any product, so <b>the lines haven't been updated</b>."
                    )
                )
                return False
            ...
            if product in unique_import_products:
                chatter_msg.append(
                    _(
                        "The product '{}' is used on several imported lines, so <b>the lines haven't been updated</b>."
            ...
            if product in existing_lines_dict:
                if uom != existing_lines_dict[product]["uom"]:
                    chatter_msg.append(
                        _(
                            "For product '{}', the unit of measure is {} on the "
                            "existing line, but it is {} on the imported line. "
                            "We don't support this scenario for the moment, so "
                            "<b>the lines haven't been updated</b>."
                        ).format(
                            product.display_name, existing_lines_dict[product]["uom"].name, uom.name
                        )
                    )
                    return False
    ...
    def _prepare_account_speed_dict(self):
        ...
        speed_dict = {}
        for account in res:
            speed_dict[account["code"].upper()] = account["id"]
        return speed_dict
        ...
        if account_dict.get("code"):
            ...
            for code, account_id in speed_dict.items():
                if code.startswith(acc_code):
                    chatter_msg.append(
                        _("Approximate match: account {} has been matched with account {}").format(
    ...
    def _prepare_analytic_account_speed_dict(self):
        ...
        speed_dict = {}
        for analytic_account in res:
            if analytic_account["code"]:
                speed_dict[analytic_account["code"].upper()] = analytic_account["id"]
        return speed_dict
    ...
    def _match_analytic_account(self, aaccount_dict, chatter_msg, speed_dict=None):
        ...
        raise self.user_error_wrap(
            ...
            ).format(aaccount_dict.get("code"))
        )

    def _prepare_journal_speed_dict(self):
        company_id = self._context.get("force_company") or self.env.user.company_id.id
        res = self.env["account.journal"].search_read([("company_id", "=", company_id)], ["code"])
        speed_dict = {}
        for journal in res:
            speed_dict[journal["code"].upper()] = journal["id"]
        return speed_dict
            ...
            if company.vat != parsed_company_vat:
                raise self.user_error_wrap(
                    _(
                        ...
                        "business document ({}) doesn't match the VAT number "
                        "of the company '{}' ({}) in which you are trying to "
                        "import this document."
                    ).format(parsed_company_vat, company.display_name, company.vat)
                )
    ...
    def post_create_or_update(self, parsed_dict, record, doc_filename=None):
        ...
        if parsed_dict.get("note"):
            if doc_filename:
                msg = _("<b>Notes in file {}:</b>").format(doc_filename)
            else:
                msg = _("<b>Notes in imported document:</b>")
            record.message_post(_("{} {}").format(msg, parsed_dict["note"]))
