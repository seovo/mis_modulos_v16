...
from .tobella_xades import PolicyId, XAdESContext, constants, template
from .tobella_xades.constants import MAP_HASHLIB, NS_MAP, EtsiNS
...
class PolicyId2(PolicyId):
    ...
    def calculate_certificate(self, node, key_x509):
        ...
        xmlsig.utils.create_node(
            "X509IssuerName", issuer_serial, xmlsig.constants.DSigNs
        ).text = get_reversed_rdns_name(key_x509.issuer.rdns)
        xmlsig.utils.create_node(
            "X509SerialNumber", issuer_serial, xmlsig.constants.DSigNs
